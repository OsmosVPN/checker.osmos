...
const CHECK_METHODS = [
  {
    id: "tcp",
    label: "Классическая проверка",
    sub: "TCP — Image + fetch пробы",
    icon: "◈",
    color: "#FFB347",
  },
  {
    id: "ws",
    label: "Проверка DNS + WebSocket",
    sub: "DNS резолв + WS handshake",
    icon: "◉",
    color: "#00F2FF",
  },
];
  ...
  return (
    ...
    >
      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
        ...
        {/* ─── Метод проверки ─── */}
        <Section label="МЕТОД ПРОВЕРКИ" color="rgba(112,255,219,0.6)">
          <div style={{ display: "flex", flexDirection: "column", gap: "7px" }}>
            ...
            >
              Результаты проверки для VLESS, VMess более точные, чем для SS,
              Trojan, HY2
            </div>
            {CHECK_METHODS.map((m) => {
              const active = settings.checkMethod === m.id;
              return (
                <MotionButton
                  key={m.id}
                  onClick={() => set("checkMethod", m.id)}
                  whileHover={{ x: 3 }}
                  whileTap={{ scale: 0.98 }}
                  style={{
                    display: "flex",
                    ...
                    transition: "all 0.2s",
                    boxShadow: active
                      ? `0 0 18px rgba(${hexToRgb(m.color)},0.12)`
                      : "none",
                  }}
                >
                  {/* Radio circle */}
                  ...
                  <div>
                    <div
                      style={{
                        fontSize: "13px",
                        fontWeight: "500",
                        color: active ? m.color : "rgba(232,244,248,0.7)",
                        transition: "color 0.2s",
                      }}
                    >
                      {m.label}
